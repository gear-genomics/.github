## GEAR Genomics: a user-friendly, open-source web platform enabling interactive genomic analysis for molecular biologists

GEAR, [https://www.gear-genomics.com/](https://www.gear-genomics.com/), is a unified, web-based genomics framework that provides a collection of lightweight, interactive applications for common molecular biology and genomics analyses directly in the browser. All code is free and open-source. The development of GEAR is funded by [de.NBI](https://www.denbi.de). Please help us improve by taking our [short user survey](https://www.surveymonkey.de/r/denbi-service?sc=hd-hub&tool=gear).

### Citation

Andreas Untergasser, Markus Hsi-Yang Fritz, Vladimir Benes, Tobias Rausch           
GEAR Genomics: a user-friendly, open-source web platform enabling interactive genomic analysis for molecular biologists                   
Nucleic Acids Res. 2026 Jul 11;54(W1):W184-W187. doi: [10.1093/nar/gkag445](https://doi.org/10.1093/nar/gkag445)
